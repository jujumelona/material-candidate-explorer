...
## 12. Self-Driving Labs (SDLs), MOF Generative Models & Phonon Stability Landscape (2024–2026 Survey)

This section documents further research breakthroughs in closed-loop robotic synthesis, Metal-Organic Framework (MOF) generative models, and MLIP-based phonon dynamical stability.

### 12.1 Closed-Loop Self-Driving Labs (SDLs) & Autonomous Robotics

| Platform / Lab | Reference | Key Contribution | Pipeline Integration |
| :--- | :--- | :--- | :--- |
| **A-Lab** | Merchant et al. / Szymanski et al., Nature 2023–2024 | Autonomous solid-state synthesis of inorganic crystals driven by generative AI | Supported SDL (`alab_solid_state`) |
| **Polybot** | Argonne National Lab (ANL) 2024 | Automated synthesis & characterization of electronic polymers & energy materials | Supported SDL (`polybot_synthesizer`) |
| **ATHENA Hub** | NSF $20M Grant (UTK / NC State) 2025–2026 | Cloud-accessible national AI self-driving lab network | Supported SDL (`athena_cloud_lab`) |
| **Opentrons Flex & Chemspeed** | 2024–2026 | High-throughput liquid & solid automated handlers | Supported SDL (`opentrons_flex`, `chemspeed_autoplatform`) |

### 12.2 MOF & Porous Material Generative Models

| Model | Reference | Key Contribution | Pipeline Integration |
| :--- | :--- | :--- | :--- |
| **MOFDiff** | ICLR 2024 | Coarse-grained equivariant graph diffusion for MOF building blocks | Supported generator (`mofdiff`) |
| **Mofasa** | 2025/2026 | All-atom latent diffusion joint sampling of atoms, positions, and lattices (up to 500 atoms) | Supported generator (`mofasa`) |
| **MolCrystalFlow** | 2026 | Periodic E(3)-invariant flow matching for molecular crystals and porous frameworks | Supported generator (`molcrystalflow`) |
| **MOFGPT** | 2024–2025 | Reinforcement learning transformer over MOFid string representations | Design reference for string MOFs |

### 12.3 Phonon Dispersion & Dynamical Stability Evaluation ($\omega^2 > 0$)

Evaluation of thermodynamic stability via formation energy ($\Delta E_f$) alone is insufficient; materials can suffer from dynamic lattice collapse. The pipeline incorporates finite displacement phonon evaluation via **Phonopy** coupled with MLIP calculators (CHGNet, MACE, MatterSim):
1. **Geometry Relaxation**: Structural relaxation to exact potential energy minimum.
2. **Supercell Displacements**: Finite atomic displacements ($0.01\text{--}0.05$ Å) generated by Phonopy.
3. **Hessian & Force Constants**: Rapid evaluation of atomic forces using MLIP foundation models.
4. **Dynamical Stability Gate**: Verification that all phonon frequencies are real ($\omega^2 > 0$) across the Brillouin zone. Imaginary frequencies ($\omega^2 < 0$) trigger an `unstable_imaginary_modes` flag.
