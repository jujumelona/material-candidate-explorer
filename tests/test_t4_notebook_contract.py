...
def test_t4_notebook_preserves_the_material_screening_contract() -> None:
    ...
    for required in (
        ...
        'OPTIMADE_API_URL = "" # @param',
        'COD_API_URL = "" # @param',
        'MP_DATABASE_VERSION_OR_RELEASE = "" # @param',
...
def test_t4_notebook_runs_all_stage_specific_evidence_routes() -> None:
    ...
    for boundary in (
        ...
        "route.research_policy_id",
        "route.research_policy_version",
        "route.required_query_intent_ids",
        "route.research_basis_ids",
        "route.runtime_escalations",
        "route.mcp_contract.accepted_arguments",
        "route.mcp_contract.allowed_record_types",
        "route.mcp_contract.required_record_fields",
        "route.mcp_contract.required_provenance_fields",
        "route.mcp_contract.required_stage_metadata_fields",
        "report.intent_coverage",
        "report.missing_intent_ids",
        '"intent_coverage"',
        '"missing_intent_ids"',
        "Completed evidence cannot omit a required query intent.",
        "persisted a bundle without intent coverage",
        "getpass.getpass",
        '"--rag-bundle"',
...
def test_t4_notebook_uses_a_bounded_external_novelty_dft_portfolio() -> None:
    ...
    for contract in (
        ...
        "build_external_novelty_lookups_from_environment()",
        '"OPTIMADE_API_URL": OPTIMADE_API_URL.strip()',
        '"COD_API_URL": COD_API_URL.strip()',
        '"COD_INCLUDE_THEORETICAL": "1" if COD_INCLUDE_THEORETICAL else "0"',
